...
function useProductService(): IProductService {
  ...
  return {
    ...
    product,
    getAll: async () => {
      productStore.setState({ products: await fetch.get(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/products`) });
    },
    getById: async (id) => {
      productStore.setState({ product: undefined });
      productStore.setState({ product: await fetch.get(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/products/${id}`) });
    },
    create: async (product) => {
      await fetch.post(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/products`, product);
    },
    update: async (id, params) => {
      await fetch.put(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/products/${id}`, params);
    },
    delete: async (id) => {
      ...
      // delete product
      await fetch.delete(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/products/${id}`);
